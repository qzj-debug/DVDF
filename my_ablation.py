...
def wandb_init(config: dict) -> None:
    wandb.init(
        ...
        group=config["group"],  # 同一参数组（如 "lr=0.01_bs=32"）
        tags=config["tag"],    # 用标签标记数据集（如 "cifar10", "mnist"）
        name=config["name"],
        resume="allow",
        reinit=True,
    )
...
if __name__ == "__main__":
    ...
    config["filter_beta"] = args.filter_beta
    ...
    for t in range(int(config['max_step'])):
        ...
        if (t + 1) % config['eval_freq'] == 0:
            ...
            wandb.log({"Score": eval_normalized_score}, step=t)  # 手动设置 step
