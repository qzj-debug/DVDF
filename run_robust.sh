...
srctypes=("random" "expert" "medium" "medium-replay" "random-expert" "medium-expert")
...
do  
    ...
    do
        for seed in "${seeds[@]}"
        do  
            tmux_name="model_${algo}_${env}_${srctype}_${seed}"
            newTmuxSession ${tmux_name}
            tmux send -t ${tmux_name} "cd /data/qzj/DVDF" C-m
            tmux send -t ${tmux_name} "conda activate o2o" C-m
            tmux send -t ${tmux_name} "python train_robust.py --algo=${algo} --env=${env} --srctype=${srctype} --seed=${seed} --device=${device} --save-model=${save_model} --filter_alpha=${filter_alpha}" C-m
        done
    done
done
