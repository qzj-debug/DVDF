...
srctypes=("medium-expert" "expert")
#"random" "medium" "expert" "medium-expert" "medium-replay" "random-expert"
...
do  
    ...
    do
        ...
        do  
            ...
            tmux send -t ${tmux_name} "python train_dmg.py --policy=${algo} --env=${env} --srctype=${srctype} --seed=${seed} --device=${device} --save_model=${save_model} --dir=${dir}  --dmglam=${dmglam} --dmgnu=${dmgnu}" C-m
        done
    done
done
